...
import { Home, Compass, Radio, Crown, User } from 'lucide-react';

export type NavTab = 'home' | 'districts' | 'voice' | 'vip' | 'profile';
...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-xl border-t border-slate-200/80 py-1.5 px-3 shadow-[0_-4px_25px_rgba(15,23,42,0.06)] select-none glossy-top-edge">
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-2xl transition-all duration-200 cursor-pointer relative active:scale-90 ${
                isActive
                  ? tab.storeHighlight 
            ...
            >
              <div className="relative p-1 rounded-xl transition-all">
                <Icon className={`w-5 h-5 transition-transform duration-200 ${
                  isActive 
                    ...
                    : 'stroke-2'
                } ${tab.storeHighlight ? 'text-amber-500' : ''}`} />
              ...
              <span className={`text-[11px] tracking-tight transition-colors ${isActive ? 'font-black' : 'font-medium'}`}>
                {tab.labelMr}
              </span>
            </button>
          );
